...
from parsed_ffmpeg.types import DataStream, FfprobeResult, BaseStream, StreamType, VideoStream, AudioStream
    ...
    for i, line in enumerate(lines):
        ...
        if stream_match:
            ...
            if stream_type == StreamType.VIDEO:
                ...
                if video_match:
                    codec, _profile, _pix_fmt, w, h, specific_bitrate_str, fps = video_match.groups()
            ...
            # --- Audio Stream Parsing ---
                ...
                if audio_match:
                    codec, sample_rate_str, channels_str, _format, specific_bitrate_str = audio_match.groups()
                    num_channels = None
                    chan_match = re.search(r"(\d+)\s*channel", channels_str, re.IGNORECASE)
                    if chan_match: num_channels = _try_int(chan_match.group(1))
                    elif channels_str.lower() == 'mono': num_channels = 1
                    elif channels_str.lower() == 'stereo': num_channels = 2
            ...
            # --- Data Stream Parsing ---
                 ...
                 data_match = re.match(r"(\w+)(?:\s*\(.*?\))?(?:,\s*(\d+\s*k?b/s))?", stream_details_raw, re.IGNORECASE)
                 if data_match:
                     codec, specific_bitrate_str = data_match.groups()
                     current_stream = DataStream(**base_stream_init_args) # Unpack args *without* 'type'
                     ...
                     if current_stream.bitrate_kbs is None: current_stream.bitrate_kbs = 0
